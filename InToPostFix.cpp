...
bool InToPostFix::parse_expr(const std::string & expression)
	{
...
//					std::cout<<"i:"<<i<<"\n";
	...
	int InToPostFix::getvalue()
	{
		return a;
	}

	// 
	void InToPostFix::infixtopostfix(const std::string & infix,Expr_Command_Factory & factory,Array <Expr_Command *> & postfix)
	{
		int k=0;
	...
	bool InToPostFix::isOperator(std::string op)
	{
		// verifying  operators 
		if(op=="+" || op=="-" || op=="*" || op=="/"|| op=="%")
		{
	...
	bool InToPostFix::isNumber(std::string op)
	{
		bool t=false;
